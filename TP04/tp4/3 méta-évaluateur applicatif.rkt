...
(define eval-letrec
  (λ (exp env*)
    (+indent)
    (let ([n-env (extend-env* (var-let exp)
                              (make-undefined-ls (var-let exp))
                              env*)])
      (for-each (λ (val var)
                  (écrit "Évaluation de ~a~%"  val)
                  
                  (set-binding*! n-env var (if (application? val)
                                               (make-procedure val n-env)
                                               val))
                  (écrit "modification, dans env ~a, du lien, maintenant ~a --> ~a ~%"  (no-env* n-env) var
                         (if (user-procedure? val) "procédure qui vient d'être retournée" val)
                         )
                  )
                (val-let exp) (var-let exp))
      (for-each (λ (body)
                (eval body n-env)
                  )
             (body-let exp))
...
(define make-undefined-ls
  (λ(ls)
    (cond
      [(null? ls) '()]
      [else(cons 'undefined (make-undefined-ls (cdr ls)))])
    ))
